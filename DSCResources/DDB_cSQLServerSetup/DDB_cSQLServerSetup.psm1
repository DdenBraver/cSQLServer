function Get-TargetResource
{
    ...
    Import-Module $PSScriptRoot\..\..\cPDT.psm1
    ...
    if($Services | Where-Object {$_.Name -eq $DBServiceName})
    {
        ...
        $Filestreamlevel = $DBServer.Configuration.FilestreamAccessLevel.configvalue
        $Filestreamsharename = $DBServer.Filestreamsharename
        $SQLSysAdminAccounts = @() 
...
function Set-TargetResource
{
    ...
    if($Filestreamlevel -gt 0)
    {
        if($PSBoundParameters.ContainsKey('Filestreamsharename'))
        {
            $Arguments += " /FILESTREAMLEVEL=`"$Filestreamlevel`""
            $Arguments += " /FILESTREAMSHARENAME=`"$Filestreamsharename`""
        }
        elseif($PSBoundParameters.ContainsKey('InstanceName'))
        {
            $Arguments += " /FILESTREAMLEVEL=`"$Filestreamlevel`""
            $Arguments += " /FILESTREAMSHARENAME=`"$InstanceName`""
        }
    }
